...
function ExecutiveLandingPage() {
  return (
    <div style={{ fontFamily: 'inherit', color: '#0a0a0a', background: '#fff' }}>
      ...
      {/* Sticky mobile CTA */}
      <style>{`
        .exec-sticky { display: flex; position: fixed; bottom: 0; left: 0; right: 0; z-index: 100; background: #fff; border-top: 1px solid #e5e5e5; padding: 0.75rem 1.25rem; align-items: center; justify-content: space-between; gap: 1rem; box-shadow: 0 -2px 12px rgba(0,0,0,0.06); }
        @media (min-width: 768px) { .exec-sticky { display: none; } }
      `}</style>
      <div className="exec-sticky">
        <p style={{ fontSize: '0.78rem', color: '#555', margin: 0, lineHeight: 1.3 }}>
          Titanium &middot; Brass &middot; Laser Engraved
        </p>
        <a
          href="#inquiry-form"
          style={{
