...
export function HeroSection() {
  ...
  return (
    <section style={{ position: 'relative', width: '100%', height: '90vh', minHeight: '540px', overflow: 'hidden' }}>
      {/* Slides — only adjacent slides stay visible for performance */}
      {SLIDES.map((slide, i) => {
        const isVisible = i === current
        const isAdjacent = i === (current + 1) % SLIDES.length || i === (current - 1 + SLIDES.length) % SLIDES.length
        return (
          <div
            key={slide.src}
            style={{
      ...
      >
        ...
        <p style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.88)', maxWidth: '36ch', lineHeight: 1.6, marginBottom: '2rem' }}>
        ...
        {/* Slide indicators — padding expands touch target to ~22px tall; visual line stays 2px */}
