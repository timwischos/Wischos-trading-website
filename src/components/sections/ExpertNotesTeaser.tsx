      ...
      >
        ...
        <div style={{ padding: '3rem 1.5rem', borderRight: '1px solid var(--grid-color)', borderBottom: '1px solid var(--grid-color)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <p style={{ fontSize: '0.75rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#767676', marginBottom: '0.75rem' }}>
            Material and Claim Discipline
          </p>
          <h2
            className="display-title"
            style={{ fontSize: 'clamp(1.5rem, 2.5vw, 2.25rem)', fontWeight: 300, lineHeight: 1.15, marginBottom: '1rem' }}
          >
            Product details written for confident B2B selling.
          </h2>
          <p style={{ fontSize: '0.875rem', lineHeight: 1.8, color: '#555', marginBottom: '1.75rem' }}>
            Brass, titanium, aluminium, stainless steel, and coated finishes behave differently in the hand, under engraving, and across production runs. Product notes focus on the details buyers need before recommending a gift to their own client.
          </p>
          <Link
          ...
          >
            Browse products with buyer notes →
          </Link>
        </div>
        ...
        <div style={{ borderBottom: '1px solid var(--grid-color)', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
          {[
            { material: 'Brass', note: 'Weight, warmth, and engraving depth make it strong for pens, desk objects, and executive gifts.' },
            { material: 'Titanium', note: 'Light, corrosion-resistant, and premium in hand. Claims are kept specific to confirmed titanium parts.' },
            { material: 'Aluminium', note: 'Lightweight, clean under anodising, and useful for desk accessories, badge holders, and stands.' },
            { material: 'Stainless Steel', note: 'Durable and consistent across volume runs, especially for drinkware and practical daily tools.' },
          ].map((item, i) => (
            <div
              key={item.material}
              style={{
